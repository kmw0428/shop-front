    ...
    const fetchProduct = async () => {
      try {
        const response = await axios.get(
          `https://shoppingback-ltd0.onrender.com/products/${id}`
        );
  ...
  const handleAddToFavorites = async (product: Product) => {
    ...
    try {
      if (favoriteProducts.includes(product.id!)) {
        // 위시리스트에서 제거
        const wish = await axios.get(`https://shoppingback-ltd0.onrender.com/wish/product/${product.id}/user/${userId}`);
    ...
    <div className="pp">
      <div className="product-page">
        <div className="product-image-container">
          <img
            src={`https://shoppingback-ltd0.onrender.com${product.imageUrl}`} // 제품에 맞는 이미지 경로 사용
            alt="Product"
